...
export const HomeDashboard: React.FC<HomeDashboardProps> = ({
  ...
  return (
    <div className="w-full pb-24 space-y-5 animate-in fade-in duration-300">
      ...
      {/* Main Grid: Responsive 1-col on mobile, 12-col bento on desktop */}
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="bg-white/95 rounded-3xl p-5 border border-sky-100 shadow-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-1">
              ...
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-100/80 col-span-2 sm:col-span-1">
                <p className="text-[11px] text-slate-400 font-medium">Legal Status</p>
                <div className="flex items-center gap-1.5 mt-1">
                  <Shield className="w-4 h-4 text-sky-600" />
                  <span className="text-xs font-bold text-slate-700 truncate capitalize">{caseData.currentStage}</span>
                </div>
                <p className="text-[10px] text-slate-500 mt-1">DLSA supported</p>
              </div>
            </div>
          </div>

          {/* Active Legal Case Snapshot Card */}
          <div className="bg-white/95 rounded-3xl p-5 border border-sky-100 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-xs font-bold text-slate-800">
                    SC/ST Protection Case Dossier
                  </h3>
                  <p className="text-[10px] text-slate-400">FIR: {caseData.caseNumber}</p>
                </div>
              </div>
              <button
            ...
            <div className="p-3 rounded-2xl bg-sky-50/50 border border-sky-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  Advocate: {caseData.districtLegalAid} (DLSA)
                </p>
              </div>
              <button
                onClick={() => onNavigateTab('case')}
                className="self-start sm:self-center px-3 py-1.5 rounded-xl bg-white hover:bg-sky-100/60 text-sky-800 border border-sky-200 text-xs font-bold transition shadow-2xs"
              >
                View Hearing Prep
              </button>
            </div>
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="bg-white rounded-3xl p-5 border border-sky-100 shadow-sm flex flex-col justify-between">
            ...
            <button
              ...
              className="w-full py-2.5 px-3 rounded-xl bg-slate-100 hover:bg-sky-50 text-slate-700 hover:text-sky-800 text-xs font-bold transition flex items-center justify-center gap-1.5"
            >
          ...
          <div className="bg-gradient-to-br from-sky-600 to-blue-700 rounded-3xl p-5 text-white shadow-md shadow-sky-200/50 flex flex-col justify-between">
            <div>
            ...
            <button
              onClick={onOpenHelpline}
              className="w-full py-2.5 px-3 rounded-xl bg-white text-sky-700 hover:bg-sky-50 text-xs font-extrabold shadow-sm transition flex items-center justify-center gap-1.5"
            >
