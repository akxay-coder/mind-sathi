...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  return (
    <div className="w-full pb-24 space-y-4 animate-in fade-in duration-300">
      ...
      {/* Header Profile Card */}
      <div className="bg-gradient-to-r from-sky-50 via-sky-50/40 to-slate-50 rounded-3xl p-5 border border-sky-100 shadow-sm flex items-center gap-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100">
            <div className="pr-2">
              ...
              <span className="text-[10px] text-slate-500 block">
                Requires fingerprint / PIN whenever reopening MindSaathi
              </span>
            </div>
            <input
              type="checkbox"
        ...
        <div className="p-3.5 rounded-2xl bg-emerald-50/70 border border-emerald-200 text-[11px] text-slate-700 space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className="px-2 py-0.5 rounded-full bg-emerald-200/80 text-emerald-900 font-semibold text-[10px]">
              {currentUser ? 'Authenticated' : 'Demo Mode Active'}
            </span>
          </div>
          <p className="text-slate-600">
            {currentUser
              ? `Signed in as ${currentUser.email || currentUser.displayName || currentUser.uid}. Cloud sync active across devices.`
      ...
      <div className="pt-2">
        ...
        >
          ...
          <span>Sign Out of MindSaathi</span>
        </button>
      </div>
    </div>
  );
